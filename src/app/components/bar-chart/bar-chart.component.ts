...
export class BarChartComponent implements OnInit, OnChanges{
  ...
  //     .domain(data.map(d => d.title.length>30? (d.title.substring(0,27)+"...") : d.title ))
  //     .range([0, height])
  //     .padding(0.2);
  ...
  //   .style('text-overflow', 'ellipsis') // Add ellipsis for truncated text
  ...
  private createChart(): void {
    ...
    const data = this.data.map(d => ({
      ...d,
      truncatedTitle: d.title.length > 30 ? d.title.substring(0, 27) + "..." : d.title
    }));
    const factor = 1; // 1->seconds, 60->minutes
    ...
    const y = d3.scaleBand()
      .domain(data.map(d => d.truncatedTitle))
      .range([0, height])
      .padding(0.2);
    ...
    svg.selectAll('rect')
      ...
      .enter()
      .append('rect')
      .attr('y', d => y(d.truncatedTitle) as number)
      .attr('width', d => x(d.duration / factor))
      .attr('height', y.bandwidth())
      .attr('fill', '#5f9ea0')
      .on('mouseover', (event, d) => {
    ...
    svg.selectAll('.label')
      ...
      .append('text')
      .attr('class', 'label')
      .attr('y', d => (y(d.truncatedTitle) as number) + y.bandwidth() / 2)
      .attr('x', d => x(d.duration / factor) + 5)
      .attr('dy', '.35em')
      .text(d => Math.trunc(d.duration / factor))
